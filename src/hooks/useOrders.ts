...
export const useOrders = (userId: number) => {
    ...
    const {
        ...
        initialPageParam: 1,
        queryFn: async () => {
            return await getOrders(userId, Number(page));
        },
    ...
    const handleSpecificPage = useCallback((pageNumber: number) => {
    ...
    const pageLinks = useMemo(() => {
        ...
        if (totalPages <= maxVisiblePages) {
            console.log(Array.from({ length: totalPages }, (_, i) => i), totalPages)
        ...
        if (currentPage <= 3) {
            ...
            return [1 - 1, ellipsis, currentPage - 1, currentPage, currentPage + 1, ellipsis, totalPages];
        }
    }, [data?.pages, page])

    return {
        data, status, handleNextPage, handlePreviousPage, hasNextPage, hasPreviousPage, handleSpecificPage, pageLinks
    }
}